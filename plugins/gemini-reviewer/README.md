...
-   **Autonomous Agent Loop & Tool Calling**: Gemini can call the host functions `get_git_file` (full file content), `search_code` (repository search, up to 100 results), `fetch_web_content` (documentation from approved domains), and `get_ci_job_log` (fetch CI job log by ID) when it needs more context than the diff.
